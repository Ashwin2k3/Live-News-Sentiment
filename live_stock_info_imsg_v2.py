...
def send_imessage(recipient, stock_ticker, news_title, news_description, news_sentiment, market_sentiment,imessage_content, news_url):
    message = f"New Update:\n\n{news_title}\n\n{news_description}\n\nNews Sentiment: {news_sentiment}\nMarket Sentiment: {market_sentiment}\n\n OverAll Sentiment:{imessage_content}\n\nRead more: {news_url}"
    script = f"""
        ...
        if articles:
            ...
            if texts:
                ...
                new_signal = get_signal_from_sentiments(sentiments)
                 ...
                 # Calculate the overall market sentiment (average polarity)
                total_polarity = sum(sentiment.polarity for sentiment in sentiments)
                avg_polarity = total_polarity / len(sentiments) if sentiments else 0

                if avg_polarity > 0:
                    overall_sentiment = "Overall Market Sentiment: Positive"
                elif avg_polarity < 0:
                    overall_sentiment = "Overall Market Sentiment: Negative"
                else:
                    overall_sentiment = "Overall Market Sentiment: Neutral"
                ...
                if new_signal != last_signal:
                    ...
                    # Send iMessage with the latest news update
                    for article in articles:
                        title = article.get('title', 'No title')
                        description = article.get('description', 'No description')
                        url = article.get('url', '#')
                        news_sentiment = analyze_sentiments(sentiment_pipeline, [description])[0].polarity
                        market_sentiment = get_signal_from_sentiments(sentiments)
                        imessage_content = f"The sentiment signal for {stock_ticker} has changed to {new_signal}. {overall_sentiment}."
                        
                        
                        send_imessage(recipient, stock_ticker, title, description, news_sentiment, market_sentiment, imessage_content, url)
